...
- [ ] Plan review loop (7 iterations) — Iter 1-6: see prior commits. Iter 7 (Codex 1 P1 + 4 P2s overlapping with Claude → 5 distinct fixes): (1) P1 Codex: `_extra_coverage` was still file-granularity after iter-6 line-pivot — intra-file unlinked hits (e.g., Depends-injected helpers in a file with other reachable branches) silently dropped. Refactored to consume `enumerated_reachable_lines` from same primitive as `_audit`; added `test_extra_coverage_includes_intra_file_unlinked_hits` regression. (2) P2 (both): "14 tests" claim was wrong — strengthened to "17 method-level tests" with breakdown. (3) P2 Codex: `_arc_source_line` docstring claims edge-cases (`"10->-1"`, bare `"10"`) but only `"10->15"` was tested. Added `TestArcSourceLine` with 3 edge-case tests. (4) P2 Claude: design spec doesn't reflect iter-6 source-line pivot — added §14 "Plan-Review Pivots" appendix to design spec covering 14.1 path format, 14.2 source-line model, 14.3 per-context API, 14.4 middleware route-template resolution. (5) P2 Claude: design spec uses `backend/src/hangman/...` paths — documented as superseded in §14.1. Iter 8 next.

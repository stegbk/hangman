...
- [ ] Plan review loop (8 iterations) — Iter 1-7: see prior commits. Iter 8 (Claude CLEAN + Codex 2 P1s + 1 P2): (1) P1 Codex: D3 stored ALL non-exit arcs in hits_by_context, but `data.arcs()` includes linear-flow arcs (e.g., `a=1; b=2` produces arc `(a_line, b_line)`). Source-line projection in E1 then treated those as branch hits — inflating totals, creating bogus extra_coverage, breaking reconciliation. Patched D3 `_arcs_for_context` to additionally filter `a[0] in branch_lines`; pre-computed `branch_lines_per_file` once in `load()` and threaded it through. Added `test_hits_exclude_non_branch_linear_flow_arcs` integration regression. (2) P1 Codex: H1 Step 7b only verified `/forfeit` shows uncovered — if Grader regressed to exact arc-id matching, BOTH endpoints would look uncovered (smoke would pass for the wrong reason). Added positive assertion that `/guesses` POSITIVELY credits apply_guess branches; sys.exit(2) on regression. (3) P2 Codex: TestArcSourceLine tests are edge-case coverage rather than behavioral regression for the iter-7 fix — clarified class docstring explaining they regression-guard the helper's contract; the iter-6 behavioral fix is covered by `test_else_arm_arc_target_still_matches_branch_at_source_line`. Iter 9 next.

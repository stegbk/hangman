...
- [ ] Plan review loop (12 iterations) — Iter 1-11: see prior commits. Iter 12 (Claude CLEAN, Codex 1 P2): H1 Step 7b prose advertised "three real codebase facts" with check 3 ("apply_guess coverage under /guesses MUST be greater than under /categories") but the Python snippet didn't compute it — said "satisfied trivially" if check 2 holds. Internal inconsistency. Removed check 3 (logically redundant with check 2 + check 1) from both prose and snippet; updated to advertise "two real codebase facts". Iter 13 next.

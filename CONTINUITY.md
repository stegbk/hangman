...
- [ ] Plan review loop (5 iterations) — Iter 1-4: see prior commits. Iter 5 (Codex 3 P1s + Claude 3 P2s, partially overlapping → 5 distinct items): (1) P1 Codex: H1 Step 7b path-format check was vacuous — only scanned `audit.unattributed_branches` + `uncovered_branches_flat`; fully-covered files slip through. Strengthened to scan `reachable_functions` + `extra_coverage` + add vacuous-PASS guard rejecting empty all_files. (2) P1 Codex / P2 Claude: D3 Step 4 stale fallback note still recommended `set_query_contexts` — replaced with kwarg-API guidance + version-pin escape hatch. (3) P1 Codex: E1 grader test fixtures used `hangman/*.py` instead of canonical `src/hangman/*.py` — replaced 45 occurrences across 5 file paths via replace_all. Added docstring on `_branch` helper documenting the canonical format. (4) P2 Claude: D3 `load()` body had dead local `arcs = data.arcs(file) or []` + stale `set_query_contexts` comment from before iter-4 — removed. (5) P2 Claude: E2 `_fixture_report` used `backend/src/hangman/game.py` (would fail H1's path-format invariant) — switched to `src/hangman/game.py` + docstring documenting why. Iter 6 next.

...
- [ ] Plan review loop (9 iterations) — Iter 1-8: see prior commits. Iter 9 (Claude CLEAN with 2 P3 nits, Codex 1 P1 + 2 P2 — first attempt hung at xhigh; retry at high reasoning_effort completed): (1) P1 Codex: `_audit` computed `extra_coverage_branches = len(extra_hit_lines)` BEFORE filtering to in-scope files — out-of-scope hit leakage would inflate the count while `_totals` and `per_file_enumerated` correctly filtered. Patched: filter `all_hit_lines` to in-scope files (keys of `total_branches_per_file`) up front. Inner `if f in per_file_enumerated` retained as belt-and-suspenders. (2) P2 Codex: test count breakdown was wrong (sum=18, not 17; referenced nonexistent `TestExtra`). Fixed with explicit per-class counts: TestPerEndpointIntersection 4, TestSharedHelperCorrectness 2, TestAuditReconciliation 4, TestTotals 2, TestArcSourceLine 5. (3) P2 Codex: `TestArcSourceLine` docstring cited `TestPerEndpointIntersection` for the iter-6 behavioral test, but it actually lives in `TestTotals`. Fixed citation. Iter 10 next.

...
- [ ] Plan review loop (11 iterations) — Iter 1-10: see prior commits. Iter 11 (Claude CLEAN, Codex 2 P1s — both codebase-grounding errors in fixtures): (1) E2 `_fixture_report` used `hangman.routes.create_game` and `hangman.game.new_game` — real handler is `start_game` (routes.py:127); `game.py` has no `new_game` function. Replaced with `hangman.routes.start_game` + `hangman.words.WordPool.random_word` (real code at words.py:22-25). (2) D7 rubric example cited `if category not in self._by_category` — real attribute is `self.categories`. Replaced with grounded reference to words.py:23 + `_assert_category_known` flow. Updated golden file assertions in TestEmit + test_renderer to match. Added §14.5 to design spec documenting the symbol-name corrections (and re-noting the iter-10 `/forfeit` fix). Iter 12 next.

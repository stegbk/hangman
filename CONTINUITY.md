...
- [ ] Plan review loop (6 iterations) — Iter 1-5: see prior commits. Iter 6 (Claude CLEAN + Codex 2 P1s + 1 P2 — 3 architectural fixes): (1) P1 Codex: D1 middleware's `_resolve_context` always took the concrete-path fallback because `request.scope["route"]` is unpopulated until AFTER middleware (Starlette dispatch order). Every path-param endpoint silently lost attribution. Switched to active route-matching against `request.app.router.routes` via `route.matches(scope)` + `Match.FULL`; added explicit `test_path_param_route_resolves_to_template` regression test. (2) P1 Codex: branch-identity model mismatched across C3↔D3↔E1 — Reachability emits synthetic `f"{line}->{line+1}"`, coverage.py emits real arc IDs (else-arms, multi-line bodies have non-`line+1` targets), AND `arc_possibilities()` includes non-branch arcs. Pivoted to source-line granularity: D3 `_authoritative_branches` → `_authoritative_branch_lines` using `Analysis.branch_lines()`; Grader added `_arc_source_line` helper, projects context_hits + all_hits to `(file, source_line)` for matching/audit/totals. Updated A3 spike to validate `branch_lines()` not `arc_possibilities()`. Added `test_else_arm_arc_target_still_matches_branch_at_source_line`. (3) P2 Codex: D3 `test_total_branches_per_file_is_authoritative` asserted `count > 0` while docstring claimed "tiny.py has exactly one branch" — strengthened to `count == 1` with diagnostic message pointing at the iter-6 fix. Iter 7 next.

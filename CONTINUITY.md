...
- [ ] Plan review loop (4 iterations) — Iter 1: 5×P1 + 3×P2 patched in `9d4b318`. Iter 2: 2×P1 patched in `b6b25b6`. Iter 3: 1×P1 patched in `42932b0`. Iter 4 (Claude + Codex, all distinct): 4×P1 patched — (a) Claude: path-format mismatch — added `relative_files = true` to `.coveragerc` so coverage.py's `measured_files()` and Reachability both produce `src/hangman/<file>.py`; (b) Codex #1: D3 `_authoritative_branches` docstring contradiction (5-tuple vs object); switched `_arcs_for_context` to design-spec'd `data.arcs(file, contexts=[label])` API; (c) Codex #2: E1 `_audit` math — properly compute `extra_branch_hits = hits.all_hits − reachable` instead of hardcoded `extra_count = 0`; (d) Codex #3: E1 `_totals` — intersect `hits.all_hits` with in-scope files instead of `len(all_hits)`. Added 2 new E1 tests + H1 path-format assertion. Iter 5 next.
